...
(() => {
  ...
  const adapter = {
    instructions: () => document.querySelector('[data-dq-instructions],[data-testid*="instruction" i],[class*="instruction" i],div.SplitPane.vertical>div.Pane.vertical.Pane1 .dq-overflow-y-auto'),
    readPane() { const e=this.instructions(); return e?.closest('div.SplitPane.vertical>div.Pane.vertical.Pane1,[data-testid*="instruction" i],section') || e; },
    editorHost() {
      const all=[...document.querySelectorAll('.CodeMirror,.cm-editor,[role="textbox"],[contenteditable="true"]')].filter(e=>!own(e));
    ...
    codePane() { const e=this.editorHost(); return e?.closest('div.SplitPane.vertical>div.Pane.vertical.Pane2,[data-testid*="workspace" i],main,section') || e; },
    action(kind) {
  ...
  const status=(message,error=false)=>{ if(state.status){state.status.textContent=message;state.status.dataset.error=String(error);} };
  ...
  const mark=()=>{
    ...
    adapter.readPane()?.setAttribute('data-dq-mobile-region','read'); adapter.codePane()?.setAttribute('data-dq-mobile-region','dq');
  };
  const mode=next=>{
...
#dq-native-editor{position:absolute!important;inset:48px 0!important;width:100%!important;height:auto!important;margin:0!important;border:0!important;border-radius:0!important;outline:none!important;resize:none!important;box-sizing:border-box!important;padding:14px!important;background:#111318!important;color:#f8fafc!important;caret-color:#fff!important;font:16px/1.45 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace!important;white-space:pre!important;overflow:auto!important;tab-size:2!important;-webkit-user-select:text!important;user-select:text!important}[data-action=run]{background:#0f766e!important}[data-action=submit]{background:#2563eb!important}`;document.head.appendChild(s);
  ...
  const boot=()=>{styles();mount();viewport();if(state.route!==key()){state.route=key();state.initialized='';}mark();if(document.body&&!document.body.dataset.dqMobileMode)mode(state.mode);const resume=[...document.querySelectorAll('button,a')].find(e=>!own(e)&&/continue here/i.test(label(e)));if(resume&&!resume.dataset.dqAutoClicked){resume.dataset.dqAutoClicked='1';resume.click();}};
